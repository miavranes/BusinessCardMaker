...
const Canvas = forwardRef(({
  ...
  useEffect(() => {
    ...
    qrEls.forEach(el => {
      QRCode.toDataURL(el.vcardString, {
        width: 512, margin: 4, errorCorrectionLevel: 'L',
  ...
  function getBounds(el) {
    if (el.type === 'text') {
      ...
      const canvas = ref?.current;
      if (!canvas) return { x: 0, y: 0, w: 60, h: 20 };
      const ctx = canvas.getContext('2d');
      ctx.font = `${el.fontStyle||'normal'} ${el.fontWeight||'normal'} ${el.fontSize}px ${el.fontFamily}`;
      const lines = (el.content || '').split('\n');
      const lh = el.fontSize * (el.lineHeight || 1.2);
    ...
    const w = (el.width  && el.width  > 0) ? el.width  : (el.imgElement?.naturalWidth  || 80);
    const h = (el.height && el.height > 0) ? el.height : (el.imgElement?.naturalHeight || 80);
    return { x: el.x, y: el.y, w: Math.max(w, 20), h: Math.max(h, 20) };
  }
  ...
  const notifySelect = useCallback((el) => {
    if (!onElementSelect || !ref?.current) return;
    const b  = getBounds(el);
    const cr = ref.current.getBoundingClientRect();
    const sx = cr.width  / CANVAS_W;
    const sy = cr.height / CANVAS_H;
    const safeW = Math.max(b.w, 40);
    const safeH = Math.max(b.h, 40);
    onElementSelect(el.id, {
      left:   cr.left + b.x * sx,
      top:    cr.top  + b.y * sy,
      right:  cr.left + (b.x + safeW) * sx,
      bottom: cr.top  + (b.y + safeH) * sy,
      width:  safeW * sx,
      height: safeH * sy,
    });
  }, [onElementSelect]); 
    ...
    if (selectedElement) {
      ...
      if (el && el.type !== 'qr') {
        ...
        for (const c of corners) {
          if (Math.hypot(x - c.cx, y - c.cy) <= 12) {
  ...
  const handleSectionResizeMouseDown = (e, section, corner) => {
    ...
    const onMove = (mv) => {
      ...
      last = { x: nx, y: ny, width: nw, height: nh };
  ...
  const handleElementResizeMouseDown = (e, el, corner) => {
    ...
    const startX = e.clientX, startY = e.clientY;
    const { x: ex, y: ey } = el;
    const bw = (el.width  && el.width  > 0) ? el.width  : getBounds(el).w;
    const bh = (el.height && el.height > 0) ? el.height : getBounds(el).h;
    const onMove = (mv) => {
  ...
  return (
    <div ref={containerRef} className="canvas-container">
      <div ref={wrapperRef} className="canvas-wrapper" style={{ position: 'relative' }}>
        <canvas
          ref={ref}
        ...
        {!LayoutComponent && (
          <div style={{
            ...
            background: backgroundColor || '#ffffff', zIndex: 0, pointerEvents: 'none',
          }} />
        )}
          ...
          return (
            <div
              ...
              onMouseDown={showPreview ? undefined : (e) => {
                e.stopPropagation();
                setSelectedElement(el.id);
                if (typeof onElementSelect === 'function' && ref?.current) {
                  const wr = ref.current.getBoundingClientRect();
          ...
          return (
            <div
              ...
              style={{
                ...
                width:  `${Math.max((b.w / CANVAS_W) * 100, 1)}%`,
                height: `${Math.max((b.h / CANVAS_H) * 100, 1)}%`,
          ...
          return (
            ...
            >
              {isSelected && ['nw','ne','sw','se'].map(corner => (
                <div
                  key={corner}
                  onMouseDown={e => handleSectionResizeMouseDown(e, section, corner)}
